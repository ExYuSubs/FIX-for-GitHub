...
if ($action == "users")
{
    ...
    Search: <input type="text" name="search" size="30" value="<?= htmlspecialchars($_GET['search'] ?? '', ENT_QUOTES, 'UTF-8') ?>" />
    <input type="submit" value="Search" />
    </form>
    </center>
...
if ($action == "forum") {
    ...
    if (isset($_POST['do']) && $_POST['do'] === 'add_this_forumcat') {
    ...
    if (isset($_POST["do"]) && $_POST["do"] === "save_edit") {        
        $id = (int) $_POST["id"];
    ...
    if (isset($_POST["do"]) && $_POST["do"] === "save_editcat") {
    ...
    if (($_POST["do"] ?? '') == "delete_forumcat" && is_valid_id($_POST["id"] ?? 0)) 
    {
